...
function xml2json(xml) {
    ...
    const namespaceUri = {};
    const aliasParser = sax.parser(true, { xmlns: true });
    aliasParser.onopentag = function (node) {
    ...
    delete namespaceUri.$current;
    aliasParser.write(xml).close();
    ...
    parser.onopentag = function (node) {
        const attributeExpressions = [
            'Binary', 'Bool', 'Date', 'DateTimeOffset', 'Decimal', 'Duration', 'EnumMember', 'Float', 'Guid', 'Int', 'String', 'TimeOfDay', 'UrlRef',
            'AnnotationPath', 'ModelElementPath', 'NavigationPropertyPath', 'PropertyPath', 'Path'];
        //TODO: check node.uri for correct namespace(s) - helper function
        //TODO: check attributes to have no namespace
        var annotatable = { target: null, prefix: '', node: node }; //TODO: node only for debugging purposes
        var annotation = {};
        ...
        switch (node.local) {
            ...
            case 'Parameter':
                ...
                annotatable.target = current.parameter;
                break;
            case 'ReturnType':
            ...
            case 'NavigationPropertyBinding': {
                ...
                var target = normalizeTarget(node.attributes.Target.value);
    ...
    parser.onclosetag = function (tag) {
        //TODO: better delete current.xxx instead of nulling?
        switch (tag) {
            case 'Edmx':
                break;
            case 'Reference':
            ...
            if (node.attributes.hasOwnProperty(name)) {
                ...
                switch (name) {
                    ...
                    case 'Type':
                        ...
                        if (type !== 'Edm.String')
                            target['$' + name] = normalizeTarget(type);
                        break;
                    case 'Version':
                ...
                if (name === 'Nullable' && !target.$Collection && node.local !== 'Singleton')
                    target['$' + name] = true;
                else if (name === 'Scale' && (target.$Type === 'Edm.Decimal' || target.$UnderlyingType === 'Edm.Decimal') && node.local !== 'Cast' && node.local !== 'IsOf')
                    target['$' + name] = 0;
                else if (name === 'Precision' && target.$Type === 'Edm.DateTimeOffset')
                    target['$' + name] = 0;
            }
        })
