...
module.exports.xml2json = function (xml, lineNumbers = false) {
    ...
    parser.onopentag = function (node) {
        ...
        switch (node.local) {
            ...
            case 'Documentation':
                annotatable.target = lastTarget;
                break;
            case 'Summary':
            case 'LongDescription':
                current.text = '';
                annotation.$Term = 'Org.OData.Core.V1.' + node.local; //TODO: summary -> description
                ...
                switch (name) {
                    ...
                    case 'DefaultValue': {
                        ...
                        if (value === 'null')
                            ...
                            target['$' + name] = false;
                        else target['$' + name] = (node.attributes[name].value === "" || isNaN(node.attributes[name].value)) ? node.attributes[name].value : Number(node.attributes[name].value);
                        break;
                    }
                    case 'EntitySet': {
