...
module.exports.xml2json = function (
  ...
  function setAttributes(target, node, names) {
    ...
    //TODO: check for unnecessary attributes if option is set
    Object.values(node.attributes)
      .filter(
        (a) =>
          a.prefix === "" &&
          a.local !== "Name" &&
          !(node.local === "Schema" && a.local === "Namespace") &&
          !(
            node.local === "Property" &&
            (a.local === "ConcurrencyMode" || a.local === "FixedLength")
          ) &&
          !(
            node.local === "FunctionImport" &&
            (a.local === "ReturnType" ||
              a.local === "EntitySet" ||
              a.local === "IsBindable" ||
              a.local === "IsSideEffecting")
          ) &&
          !(node.local === "Parameter" && a.local === "Mode") &&
          !names.includes(a.local)
      )
      .forEach((attribute) => {
        throw new Error(
          `Element: ${node.local}, unexpected attribute: ${attribute.local}`
        );
      });
    if (annotations) {
