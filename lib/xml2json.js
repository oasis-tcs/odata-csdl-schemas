/**
 * Converts OData CSDL XML 4.0x to OData CSDL JSON
*/
    ...
    preParser.onopentag = function (node) {
        switch (node.local) {
            ...
            case 'Schema':
                ...
                break;
            case 'Association':
                preV4.current.association = {};
                preV4.association[preV4.current.namespace + '.' + node.attributes.Name.value] = preV4.current.association;
                break;
            case 'AssociationSet':
                preV4.current.associationSet = {};
                //TODO: probably better keyed by association name
                preV4.associationSet[node.attributes.Name.value] = preV4.current.associationSet;
                break;
            case 'End':
                if (preV4.current.association) {
                    preV4.current.association[node.attributes.Role.value] = {
                        type: normalizeTarget(node.attributes.Type.value),
                        mult: node.attributes.Multiplicity.value
                    };
                } else if (preV4.current.associationSet) {
                    preV4.current.associationSet[node.attributes.Role.value] = node.attributes.EntitySet.value;
                }
                break;
        }
    }
        ...
        switch (node.local) {
            ...
            case 'NavigationProperty':
                ...
                if (result.$Version < '4.0') {
                    //TODO: alias -> namespace
                    const relationship = node.attributes.Relationship.value
                    const association = preV4.association[relationship];
                    ...
                    //TODO: $Partner = //edm2:NavigationProperty[@Relationship=$relation and @FromRole=$torole]/@Name
                    //TODO: does this need a loop over the associations before returning the result?
                    //TODO: maybe same for annotation sets to $NavigationPropertyBinding
                } else {
            ...
            case 'FunctionImport':
                ...
                if (result.$Version < '4.0') {
                    ...
                    //TODO: overload ReturnType with facets
            ...
            case 'Annotation':
                //TODO: other annotation attributes
