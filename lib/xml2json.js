...
module.exports.xml2json = function (
   ...
   * @param {Object} node The current XML node
   */
  function checkXmlNamespace(node) {
    const schema = SCHEMA[node.local] || {};
  ...
  parser.onopentag = function (node) {
    ...
    current.element.unshift(node.local);
    ...
    switch (node.local) {
      ...
      case "EntityType":
        ...
        if (current.schema[node.attributes.Name.value])
          recordError("Type name collides with other schema child", this);
      ...
      case "Action":
        ...
        else recordError("Action name collides with other schema child", this);
      ...
      case "Function":
        ...
        if (Array.isArray(current.schema[node.attributes.Name.value]))
          ...
          recordError("Function name collides with other schema child", this);
        annotatable.target = current.overload;
        break;
      case "Parameter": {
        checkAttribute(node, "Name");
      ...
      case "Term": {
        ...
        if (current.schema[node.attributes.Name.value])
          recordError("Term name collides with other schema child", this);
  ...
  parser.ontext = function (text) {
    if (current.text === "") current.text = text;
      ...
      throw new Error(
        `Element ${current.element[0]}, unexpected text: ${text}`
      );
  };
   ...
   */
  function recordError(message, parser) {
