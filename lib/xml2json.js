...
module.exports.xml2json = function (
    ...
    switch (node.local) {
      ...
      case "End":
        ...
        if (preV4.current.association) {
          ...
          preV4.current.associationEnd.mult =
  ...
  const SCHEMA = {
    ...
    Include: { x: true, Annotation: { min: 0 } },
    ...
    Annotation: { expression: { min: 0 }, Annotation: { min: 0 } },
    ...
    PropertyValue: { expression: { min: 0 }, Annotation: { min: 0 } },
    UrlRef: {
      isExpression: true,
      expression: { min: 0, max: 1 },
      Annotation: { min: 0 },
  ...
  function checkElementAllowed(node) {
    ...
    // increment occurence in parent, check against max occurrence in parent's schema
    ...
    if (
      (schema.x && result.$Version >= "4.0" && node.uri !== EDMX) ||
      (schema.x && result.$Version < "4.0" && node.uri !== EDMX_V1) ||
      (!schema.x && result.$Version >= "4.0" && node.uri !== EDM) ||
      (!schema.x && result.$Version < "4.0" && !EDM_OLD.includes(node.uri))
    )
      throw new Error(
  ...
  function setAttributes(target, node, extract, ignore = []) {
      ...
      if (node.attributes[name]) {
        switch (name) {
          ...
          case "DefaultValue": {
            ...
            else if (value === "true") target["$" + name] = true;
            else if (value === "false") target["$" + name] = false;
            else
              target["$" + name] =
          ...
          case "Scale":
            if (
              ...
              target["$" + name] = isNaN(node.attributes[name].value)
                ? node.attributes[name].value
                : Number(node.attributes[name].value);
            break;
          case "ReturnType":
    ...
    Object.values(node.attributes)
      ...
      .forEach((attribute) => {
        throw new Error(
          `Element: ${node.name}, unexpected attribute: ${attribute.name}`
        );
      });
    if (annotations) {
