...
module.exports.xml2json = function (
  ...
  preParser.onopentag = function (node) {
    if (!checkElementAllowed(node, this)) return;

    switch (node.local) {
      case "Edmx":
        checkAttribute(node, "Version");
        result.$Version = node.attributes.Version.value;
        break;
      case "DataServices": {
  ...
  preParser.onclosetag = function (tag) {
    ...
    if (!checkElementComplete(local, this)) return;
  ...
  const SCHEMA = {
    ...
    Schema: {
      ...
      // V2/V3
      Association: { min: 0 },
      // Using -> Reference with dummy Uri + Include
      // ValueTerm -> Term
    },
    Annotations: {
      v2x: true,
      Annotation: { min: 0 },
      // V3
      // TypeAnnotation -> Annotation + Record
  ...
  const CSDL_NAMESPACES = [EDMX, EDM, EDMX_V1, ...EDM_OLD];
   ...
   * @return {boolean} element has the correct minimum number of each child element
   */
  function checkElementAllowed(node, parser) {
    const parent = elements[0];
    elements.unshift({ local: node.local });

    // skip foreign markup in V2/V3
    if (result.$Version < "4.0" && !CSDL_NAMESPACES.includes(node.uri)) {
    ...
    if (!parentSchema || !parentSchema[childName]) {
      if (parser && parent.local === "")
        throw new Error(`Unexpected root element: ${node.local}`);
      else if (parser)
        reportError(
  ...
  /**
   * Check if current element is skipped
   * @return {boolean} skip current element
   */
  function checkElementSkipCurrent() {
    return elements[0].skip;
  }

  /**
   * Check XML namespace of element
   * @param {Object} node The current XML node
   * @param {Object} parser XML parser instance
   * @return {boolean} namespace is ok
   */
   ...
   * @return {boolean} element has the correct minimum number of each child element
   */
  function checkElementComplete(local, parser) {
    const element = elements.shift();

    if (element.skip) return false;
  ...
  parser.onopentag = function (node) {
    ...
    switch (node.local) {
      ...
      case "EntityType":
        ...
        v2Annotations(current.type, node, SAP_V2_ENTITY_TYPE);
        if (current.schema[node.attributes.Name.value])
          reportError("Type name collides with other schema child", this);
      ...
      case "Action":
        ...
        else reportError("Action name collides with other schema child", this);
      ...
      case "Function":
        ...
        if (Array.isArray(current.schema[node.attributes.Name.value]))
          ...
          reportError("Function name collides with other schema child", this);
        annotatable.target = current.overload;
        break;
      case "Parameter": {
        if (!current.overload) {
          reportError("Unexpected element: Parameter", this);
          break;
        }
        checkAttribute(node, "Name");
        checkAttribute(node, "Type");
      ...
      case "Term": {
        ...
        if (current.schema[node.attributes.Name.value])
          reportError("Term name collides with other schema child", this);
      ...
      case "Dependent":
        if (result.$Version >= "4.0")
          reportError(`Unexpected element: ${node.local}`, this);
        break;
    }
  ...
  parser.ontext = function (text) {
    ...
    else if (text.trim() !== "" && !checkElementSkipCurrent()) {
      reportError(`Element ${this.tag.name}, unexpected text: ${text}`, this);
    }
  };

  parser.oncdata = function (text) {
    if (current.text == null)
      reportError(`Element ${this.tag.name}, unexpected CDATA: ${text}`, this);
    else {
      current.text += text;
    }
  };
        ...
        if (current.annotatable[1].target != null) {
          ...
          if (
            ...
            try {
              ...
              reportError(`Element ${this.tag.name}, invalid JSON`, this);
            }
          }
  ...
  function checkAttribute(node, name) {
    if (!node.attributes[name])
      // can't continue processing without this attribute
  ...
  /**
   * Record error: add to messages, throw if strict
   * @param {string} message Description of the error
   * @param {Object} parser XML parser instance
   */
  function reportError(message, parser) {
    if (strict) throw new Error(message);
    messages.push(enrichedException({ message }, xml, parser));
  }
  ...
  const SAP_V2_ENTITY_TYPE = {
    label: {
      vocabulary: "Core",
      term: "Description",
    },
  };
  const SAP_V2_PARAMETER = {
    label: {
      vocabulary: "Core",
      term: "Description",
    },
